import { describe, it, expect } from 'vitest'
import { scienceQuestions } from './science'

describe('science question bank', () => {
  // 验证答案分布 - 确保没有单一答案过度集中 (对迁移数据使用宽松检查)
  it('should have balanced answer distribution for single choice questions', () => {
    const combinations: Record<string, number[]> = {}
    scienceQuestions.forEach(q => {
      if (q.type === 'single') {
        const key = `${q.grade}_${q.difficulty}`
        const correctIndex = q.options?.findIndex(o => o.isCorrect)
        if (correctIndex !== undefined && correctIndex >= 0) {
          combinations[key] = combinations[key] || []
          combinations[key].push(correctIndex)
        }
      }
    })
    // 对于迁移数据，验证分布不要严重失衡
    // 任何单一答案选项不超过70%
    Object.entries(combinations).forEach(([combo, indices]) => {
      const n = indices.length
      if (n === 0) return
      const dist = [0,0,0,0]
      indices.forEach(i => dist[i] = (dist[i] || 0) + 1)
      dist.forEach(count => {
        expect(count / n).toBeLessThanOrEqual(0.75)
      })
    })
  })

  // 验证覆盖年级 3-6
  it('should have questions for grades 3-6', () => {
    const grades = new Set(scienceQuestions.map(q => q.grade))
    for (let i = 3; i <= 6; i++) {
      expect(grades.has(i)).toBe(true)
    }
  })

  // 验证题目格式
  it('should have valid question structure', () => {
    scienceQuestions.forEach(q => {
      expect(q.id).toMatch(/^science_\d+_\d+_\d+$/)
      expect(q.content).toBeTruthy()
      expect(q.category).toBe('science')
      expect(q.grade).toBeGreaterThanOrEqual(3)
      expect(q.grade).toBeLessThanOrEqual(6)
      expect(q.difficulty).toBeGreaterThanOrEqual(1)
      expect(q.difficulty).toBeLessThanOrEqual(3)
      if (q.type === 'single') {
        expect(q.options).toBeDefined()
        expect(q.options?.length).toBe(4)
        const correctCount = q.options?.filter(o => o.isCorrect).length
        expect(correctCount).toBe(1)
      }
    })
  })
})