...
> 注：历史为新增学科，难度1-2为基础历史知识，难度3涉及古代文明、考古知识
...
| ID | 名称 | 难度 | 位置(x,z) | 钥匙需求 | 连接 |
|----|------|------|----------|---------|------|
| south_treasure_1 | 火山文明遗迹 | 1 | (4,7) | 2 | south_history_2（特例：连接历史链进阶岛屿，作为进阶奖励）|
...
**Boss解锁规则：**
- Boss区域连接到所有难度3的普通区域（玩家可到达但有前置条件）
- 必须先击败12个普通区域岛屿（每科至少击败1个）才能进入Boss区域
- 具体前置条件由 `isAreaReachable` 函数实现
