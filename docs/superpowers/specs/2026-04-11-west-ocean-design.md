...
**同级互联规则：**
- 同一难度等级的不同学科岛屿之间横向互联
- 例如：west_math_1 ↔ west_chinese_1 ↔ west_english_1 ↔ west_science_1

**纵向升级规则：**
- 同一学科内，难度N岛屿连接到难度N+1岛屿
- 例如：west_math_1 → west_math_2 → west_math_3

**Boss解锁规则：**
- Boss区域（west_boss）连接到所有难度3的普通区域
- 必须先击败9个普通区域岛屿才能挑战Boss

**隐藏区域规则：**
- 隐藏区域连接相邻学科的第1级岛屿
- 例如：west_hidden_A连接west_math_1
...
### 6.1 数据文件
- [ ] `src/data/areas/index.ts`：添加westAreas数组（16个区域：12普通+2隐藏+1宝藏+1Boss）
